...
class DatabaseController {
  ...
  Future<void> insertGenres(List<Genre> genres) async {
    ...
    for (var genre in genres) {
      batch.insert('genresTable', genre.toJson(),
  ...
  Future<List<Genre>> getAllGenres() async {
    ...
    return result.isNotEmpty
        ? result.map((e) => Genre.fromJson(e)).toList()
        : [];
  }

  Future<int> updateGenre(Genre genre) async {
    final db = await dbClient.db;
    return db.update('genresTable', genre.toJson(),
  ...
  Future<List<Story>> getAllStories({List<String>? columns}) async {
    final db = await dbClient.db;
    var result = await db.query("storiesTable",
        columns: columns, orderBy: "updatedAt DESC");
    return result.isNotEmpty
        ? result.map((e) => Story.fromJson(e)).toList()
        : [];
  }

  Future<List<Story>> searchStories(
      {List<String>? columns, required String query}) async {
    final db = await dbClient.db;
  ...
  Future<int> countStory(String status) async {
    ...
    final count = Sqflite.firstIntValue(
      await db.rawQuery(
          "SELECT COUNT(*) FROM storiesTable WHERE status = '$status'"),
    );
  ...
  Future<int> updateStory(Story story) async {
    ...
    return await db.update("storiesTable", story.toJson(),
  ...
  Future<Story?> getStoryById(String id) async {
    final db = await dbClient.db;
    var result = await db
        .query("storiesTable", limit: 1, where: "id = ?", whereArgs: [id]);
    return result.isNotEmpty
        ? result.map((e) => Story.fromJson(e)).toList()[0]
        : null;
  }
  ...
  Future<int> saveReadingProgress(
      String chapterUrl, double scrollOffset) async {
    final db = await dbClient.db;
    return db.insert(
      'reading_progress',
      {
  ...
  Future<double> getReadingProgress(String chapterUrl) async {
    ...
    if (result.isNotEmpty) {
      final scrollOffset = result.first['scrollOffset'];
      if (scrollOffset != null) {
        return (scrollOffset as num).toDouble();
      }
    }
    return 0.0;
  }
